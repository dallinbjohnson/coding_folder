...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    @backmusic = Gosu::Song.new("media/backmusic.m4a")
    @backmusic.play(true)
    
    @sounds = []

    asteroid = ["media/AsteroidBig.png","media/AsteroidMedium.png","media/AsteroidSmall.png"]
    @asteroid_anim = Gosu::Image.load_tiles("media/star.png", 25, 25, :tileable => true)
    @asteroid = Array.new

    @star = Gosu::Image.load_tiles("media/star.png", 25, 25, :tileable => true)
    @x = rand * 639
    @y = 0
  end
  ...
  def update
    ...
    if rand(100) < 4 and @asteroid.size < 4
      @asteroid.push(Asteroid.new(@asteroid_anim))
    end

    @asteroid.each { |asteroid| asteroid.move }
    # binding.pry
    @ship.collect_stars(@asteroid) unless @ship.can_collect_stars()
  ...
  def draw
    ...
    @font.draw("Score: #{@ship.score}", 0, 45, ZOrder::UI, 1.0, 1.0, Gosu::Color::YELLOW)
        
    @background_image.draw(0, 0, ZOrder::Background) 
    @ship.draw

    @asteroid.each { |asteroid| asteroid.draw }

    # img = @star[Gosu.milliseconds / 100 % @star.size]
    # img.draw(@x - img.width / 2.0, @y, ZOrder::Asteroid, 1, 1)
  end
...
class Ship
  ...
  def initialize(window)
    @ship_image = Gosu::Image.new('media/starfighter.bmp', :tileable => false)
    @shield_image = Gosu::Image.new("media/shield.png", :tileable => false)
    @window = window
    @x = window.width / 2
    @y = window.height / 2
    @speed = 5
  ...
  def update
    ...
    @nuke_counter += 1
    unless @shots.each {|s| s.class == Nuke }
      binding.pry
      @nuke_counter = 0
    end
  end
  ...
  def can_collect_stars
    @shots.empty?
  end

  def collect_stars(stars)
    newShots = @shots.flatten

    newShots.each do |shot|
      stars.reject! do |star|
        binding.pry
        if Gosu.distance(shot.x, shot.y, star.x, star.y) < 15
          # binding.pry
          @score += 10
          # @beep.play
          true
        elsif shot.class == Nuke && Gosu.distance(shot.x, shot.y, star.x, star.y) < @nuke_counter
          @score += 10
          # @beep.play
          true
        else
...
class BaseShot
  ...
  def self.load_sound(window)
    @fire_sound ||= Gosu::Sample.new('media/fire.ogg')
  end
...
class Nuke < BaseShot
  ...
  def self.load_image(window)
    @loaded_image ||= Gosu::Image.new("media/shield.png", :tileable => false)
  end
...
class Asteroid
  ...
  def draw  
    ...
    img.draw(@x - img.width / 2.0, @y, ZOrder::Asteroid, 1, 1, @color, :add) # , @color, :add
  end
end
